...
export const MedalFlowSankey: FC<{ topCountries: MedalData[]; selectedCountry: string | null; selectedMedalType: 'all' | 'gold' | 'silver' | 'bronze' }> = ({ topCountries, selectedCountry, selectedMedalType }) => {
  ...
  useEffect(() => {
    const loadData = async () => {
      ...
      try {
        ...
        const normalizeCountry = (value: string) => getCountryCode(value);
        const selectedCountryCode = selectedCountry ? normalizeCountry(selectedCountry) : null;
        const countries = selectedCountryCode
          ? [selectedCountryCode]
          : topCountries.map(c => normalizeCountry(c.country));
        const sports = SPORT_FILES.map(f => f.replace(/\.csv$/i, '')).slice(0, 10);
        const medalTypes = ['Gold', 'Silver', 'Bronze'];
        const medalTypeMap: Record<'gold' | 'silver' | 'bronze', string> = {
          gold: 'Gold',
          silver: 'Silver',
          bronze: 'Bronze'
        };
        const activeMedalTypes = selectedMedalType === 'all'
          ? medalTypes
          : [medalTypeMap[selectedMedalType]];

        const getCountry = (row: typeof result.data[string][number]) =>
          normalizeCountry(row.participant_country || '');
        ...
        const nodes: SankeyNode[] = [
          ...
          ...activeMedalTypes.map(m => ({ name: m, category: 'medal' as const }))
        ];
        ...
        countries.forEach(country => {
          sports.forEach(sport => {
            const value = result.data[sport]
              ?.filter(d => {
                if (getCountry(d) !== country) return false;
                const medalType = getMedalType(d);
                if (selectedMedalType === 'all') return medalType !== '';
                return medalType === medalTypeMap[selectedMedalType];
              })
              .length ?? 0;
            if (value > 0) {
              links.push({
        ...
        sports.forEach(sport => {
          activeMedalTypes.forEach(medal => {
            const value = result.data[sport]
    ...
    loadData();
  }, [topCountries, selectedCountry, selectedMedalType]);
  ...
  useEffect(() => {
    ...
    const highlightNode = (node: any) => {
      ...
      linkSelection
        ...
        .attr('opacity', (link: any) =>
          link.source.index === node.index || link.target.index === node.index ? 0.9 : 0.15
        );
    ...
    const highlightLink = (link: any) => {
      ...
      linkSelection
        .transition()
        .duration(200)
        .attr('opacity', (d: any) => (d === link ? 0.9 : 0.15));
    ...
    linkSelection
      .on('mouseover', (event, d: any) => {
        ...
        tooltip
          .style('opacity', 1)
          .html(`<strong>${d.source.name}</strong> → <strong>${d.target.name}</strong><br/>${d.value} medals`);
        const [xPos, yPos] = d3.pointer(event, containerRef.current);
        tooltip
          .style('left', `${xPos + 12}px`)
          .style('top', `${yPos}px`);
    ...
    nodeSelection
      .attr('opacity', d => (selectedCountry && d.category === 'country' && d.name !== selectedCountry ? 0.25 : 1));

    linkSelection
      .attr('opacity', d => (selectedCountry && d.source.category === 'country' && d.source.name !== selectedCountry ? 0.15 : 0.6));
